...
st.set_page_config(page_title="Score Match Diffusion", layout="wide")
st.markdown("""
<style>
...
st.markdown('<div class="main-header">🎯 Score Match Diffusion Engine</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">Stein score matching | Langevin dynamics | Score = ∇ log p(x) | Local momentum / mean‑reversion signal | Multi‑window selection</div>', unsafe_allow_html=True)

st.sidebar.markdown("## 🎯 Score Match Diffusion")
st.sidebar.markdown(f"**Run Date:** `{st.session_state.get('run_date', 'Not loaded')}`")
st.sidebar.markdown(f"**Next Trading Day:** `{next_trading_day()}`")
st.sidebar.markdown(f"**Noise scale:** {config.NOISE_SCALE}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'score_match_diff_' in f]
    if not json_files:
        return None
...
with st.expander("📖 Interpretation", expanded=True):
    st.markdown("""
    - **Score** = gradient of the log‑probability density of ETF returns (∇ log p(x)).
    ...
    - **Magnitude** indicates the strength of the local drift – larger absolute score = stronger signal.
    - The network is trained using **denoising score matching** on the joint distribution of ETF returns over a rolling window.
    - For each ETF, the rolling window that gives the **highest absolute score** is selected.
    - Rank by absolute score → larger magnitude = more significant local movement.
    """)
...
st.caption("Score match diffusion trains a neural network to estimate ∇ log p(x) using denoising score matching. The score at the current return vector gives the direction of highest probability density gradient. Larger absolute score = stronger local momentum / mean‑reversion signal.")
